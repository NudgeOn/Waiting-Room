...
function Editor({room,existing,busy,onSave,canWrite,profile}){
  ...
  return <form className="control-card" ref={form} noValidate onSubmit={submit}>
    ...
    <p className="control-warning">초안 저장만 수행합니다. 원본 연결 검사·서명 배포·대기열 활성화는 실행하지 않습니다. Quick 20 자동 검증은 아직 연결되지 않았습니다.</p>
...
export default function Control({session,csrf,onBack,onEnrollment,onSessionChanged,persistent=false,route={page:'rooms'}}){
  const [snapshot,setSnapshot]=useState(null),[audit,setAudit]=useState([]),[selection,setSelected]=useState(()=>route.page==='new'?{room:newRoom(),existing:false}:null),[busy,setBusy]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState(()=>history.state?.wrNotice==='draft-saved'?'초안을 저장했습니다. 실제 대기열에는 아직 적용하지 않았습니다.':'');
  ...
  async function load(){if(lock.current)return;lock.current=true;setBusy(true);setError('');try{const [config,events]=await Promise.all([controlAPI('/config/draft'),controlAPI('/audit-events')]);if(mounted.current){setSnapshot(config);setAudit(events.data.items);if(route.page!=='new')setSelected(null);pending.current=null;setNotice('최신 저장 초안을 불러왔습니다.');}}catch(e){if(mounted.current)setError(e.message);}finally{lock.current=false;if(mounted.current)setBusy(false);}}
