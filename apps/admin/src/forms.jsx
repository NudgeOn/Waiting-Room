...
export function Enrollment({pending,account,perform,accept,back,busy}){
  ...
  return <><h2>TOTP 등록</h2><p className="intro">인증 앱에 등록한 뒤 6자리 코드로 확인하세요.</p>{!setup?<button className="primary" disabled={busy} onClick={()=>perform(async()=>setSetup(await api('/auth/totp/enroll',{proof:pending.challengeToken,body:{}})))}>등록 키 만들기</button>:<>
    ...
    <form onSubmit={e=>{e.preventDefault();const form=e.currentTarget;const code=new FormData(form).get('code');perform(async()=>{const out=await api('/auth/totp/enroll/verify',{proof:pending.challengeToken,body:{code}});form.reset();setSetup(null);setQR('');accept(out);});}}><Field label="인증 코드" name="code" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} autoComplete="one-time-code"/><Submit busy={busy}>등록 완료</Submit></form>
...
export function Session({session,csrf,perform,logout,openControl}){
  return <><h2>관리자 세션</h2><p className="intro">서버에서 확인한 현재 인증 상태입니다.</p><dl className="session"><dt>사용자</dt><dd>{session.userId}</dd><dt>역할</dt><dd>{session.role}</dd><dt>MFA 인증</dt><dd>{session.mfaVerified?'완료':'TOTP OFF'}</dd><dt>세션 만료</dt><dd>{new Date(session.idleExpiresAt).toLocaleString('ko-KR')}</dd></dl>
    <p className="notice">로컬 개발 환경입니다. Room 초안은 저장할 수 있지만 실제 대기열 운영·설치 배포는 아직 연결되지 않았습니다.</p>
