...
function App(){
  ...
  if(phase==='session'&&info.draftAuthoring){
    ...
    return <div className="shell control-shell"><header><Link className="wordmark" to="/">Waiting Room</Link><span>관리자 콘솔</span></header><main>
      <nav className="workspace-nav" aria-label="콘솔 화면"><Link to="/" aria-current={dashboard?'page':undefined}>대시보드</Link><Link to="/rooms" aria-current={['room','rooms','new','runtime'].includes(route.page)?'page':undefined}>Rooms</Link>{info.persistent&&session.capabilities.some(c=>c.action==='security.read')?<Link to="/settings" aria-current={route.page==='security'?'page':undefined}>사용자 · 보안</Link>:null}</nav>
      {dashboard?<><Dashboard persistent={info.persistent===true} canWrite={Boolean(csrf)&&session.capabilities.some(c=>c.action==='config.write'&&!c.requiresReauthentication)}/><section className="control-card">{sessionPanel}{error?<p className="error" role="alert">{error}</p>:null}</section></>:route.page==='not-found'?<section><h2>화면을 찾을 수 없습니다.</h2><Link to="/">대시보드로 돌아가기</Link></section>:<>
        {route.page==='room'?<RoomTabs id={route.roomId} tab={route.tab}/>:null}
        <Control key={path} route={route} session={session} csrf={csrf} persistent={info.persistent===true} onEnrollment={out=>accept(out,session.userId)} onSessionChanged={syncSession} onBack={()=>navigate('/')}/>
      </>}
    </main></div>;
  }
  return <div className="shell"><header><span className="wordmark">Waiting Room</span><span>관리자 콘솔</span></header><main><section className="welcome"><h1>흐름은 차분하게,<br/>운영은 간편하게.</h1><p>사이트와 앱의 대기열을<br/>한곳에서 안전하게 관리하세요.</p><small>Apache-2.0 · Self-hosted</small></section><section className="form-surface" ref={heading} tabIndex={-1} aria-label="관리자 인증">
    ...
    {phase==='session'?sessionPanel:null}
