...
function App(){
  ...
  if(phase==='session'&&info.draftAuthoring){
    ...
    return <div className="shell control-shell"><a className="skip-link" href="#console-content">본문으로 건너뛰기</a><header><Link className="wordmark" to="/" aria-label="NudgeOn Waiting Room"><Brand/></Link><span>관리자 콘솔</span></header><main id="console-content" tabIndex={-1}><h1 className="sr-only">NudgeOn Waiting Room 관리자 콘솔</h1>
      <nav className="workspace-nav" aria-label="콘솔 화면"><Link to="/" aria-current={dashboard?'page':undefined}>대시보드</Link><Link to="/rooms" aria-current={['room','rooms','new','runtime'].includes(route.page)?'page':undefined}>대기열</Link>{info.trafficLab&&session.capabilities.some(c=>c.action==='lab.read')?<Link to="/traffic-lab" aria-current={route.page==='traffic-lab'?'page':undefined}>테스트</Link>:null}{info.persistent&&session.capabilities.some(c=>c.action==='security.read')?<Link to="/settings" aria-current={route.page==='security'?'page':undefined}>사용자 · 보안</Link>:null}</nav>
  ...
  return <div className={phase==='setup'&&info.setupWizard?'shell setup-shell':'shell'}><header><span className="wordmark"><Brand/></span><span>관리자 콘솔</span></header><main><section className="welcome"><h1>흐름은 차분하게,<br/>운영은 간편하게.</h1><p>사이트와 앱의 대기열을<br/>한곳에서 안전하게 관리하세요.</p><small>Apache-2.0 · Self-hosted</small></section><section className="form-surface" ref={heading} tabIndex={-1} aria-label="관리자 인증">
    ...
    {phase==='session'?<>{sessionPanel}{info.setupWizard?<><p className="notice">초기 설정이 완료되었습니다. 관리자 콘솔에서 다시 로그인하면 대기열을 만들고 운영할 수 있습니다.</p><button className="secondary" disabled={busy} onClick={()=>perform(async()=>{await logout();location.assign(info.adminUrl+'/auth/login');})}>설정 완료 · 관리자 콘솔로 이동</button></>:null}</>:null}
    {error?<p className="error" role="alert">{error}</p>:null}
  </section></main><footer><span>NudgeOn Waiting Room</span><span>로그인 정보는 이 서버에서만 처리됩니다.</span></footer></div>;
}
