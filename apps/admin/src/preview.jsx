      ...
      <section className="preview-panel" aria-busy={busy}>
        ...
        {step<4?<form ref={form} noValidate onSubmit={event=>{event.preventDefault();step===3?void submit('plan'):advance();}}><fieldset disabled={busy}>
          ...
          {step===2?<>
            ...
            <div className="preview-grid"><Field label="동시에 유지할 입장권 수" name="maxActiveAdmissionLeases" type="number" min={1} max={cap} value={draft.limits.maxActiveAdmissionLeases} error={fieldErrors.maxActiveAdmissionLeases} hint="동시에 유효한 입장권의 상한 · 현재 접속자 수와 다릅니다" onChange={e=>edit({...draft,limits:{...draft.limits,maxActiveAdmissionLeases:numberValue(e)}})}/>
              <Field label="1분당 입장 허용 인원" name="admissionsPerMinute" type="number" min={1} max={rate} value={draft.limits.admissionsPerMinute} error={fieldErrors.admissionsPerMinute} hint="원본 서비스가 처리할 수 있는 속도에 맞춰 설정하세요" onChange={e=>edit({...draft,limits:{...draft.limits,admissionsPerMinute:numberValue(e)}})}/></div>
