...
export default function ReauthDialog({request,csrf,requireTOTP,onComplete,onCancel}) {
  ...
  return <dialog ref={dialog} className="reauth-dialog" aria-labelledby="reauth-title" onCancel={event=>{event.preventDefault();if(!locked.current)onCancel();}}>
    ...
    <form onSubmit={submit}><fieldset disabled={busy}><legend>현재 Admin 확인</legend>{!retry?<>
      ...
      <p className="control-help">이미 로그인에 사용한 TOTP 코드는 재사용할 수 없습니다. 인증 앱의 다음 코드를 기다려 주세요.</p>
