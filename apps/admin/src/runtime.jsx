...
export default function RuntimeDashboard({csrf,session,draft,onBack,roomId,tab}){
 ...
 {tab==='operations'?<section className="control-card"><h3>{room.name} · 유량과 모드</h3><p className="ops-description">{room.hostname} · 활성 입장권 최대 {runtime.limits.maxActiveAdmissionLeases.toLocaleString('ko-KR')}개</p><RoomHealth data={telemetry}/><RuntimeState data={telemetry}/><QueueMetrics data={telemetry}/>
 <p className="ops-metrics-note"><strong>입장 완료는 유효 입장권 수입니다.</strong> READY 예약을 제외한 값이며 현재 접속자 수나 누적 방문자 수가 아닙니다. 실제 입장 배정은 최근 60초의 READY 예약 발급 수입니다.</p>
 ...
 <LimitsForm key={room.id} room={room} runtime={runtime} profile={view.config.profile} busy={busy||unobserved} onCommand={command}/></>:null}{session.capabilities.some(c=>c.action==='runtime.instant_off')?<button className="secondary" disabled={busy||!room.active||Boolean(refreshError)} onClick={()=>instantOff(room,runtime)}>재인증 후 즉시 OFF</button>:null}</section>:null}
