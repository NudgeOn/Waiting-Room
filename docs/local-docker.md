...
읽기 응답만 유실된 경우 해당 요청은 503이며, 클라이언트는 표시된 재시도 간격 뒤 같은
대기표로 다시 요청한다. 이것만으로 설치 전체의 불확실한 쓰기 상태를 만들지 않는다.
기존 대기표를 지우거나 서버를 재시작할 필요는 없다. 재시도에서도 서버의 primary와
fence 검증은 유지한다. [수정 전후 검증](evidence/beta-20260909-read-recovery.md).
