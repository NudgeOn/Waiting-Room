...
상태 조회가 `ready` 또는 `admitted`를 반환하면 입장 버튼 없이 자동 이동 안내를 표시하고,
3초 후 기존 서명된 입장 폼을 한 번 전송한다. Gateway가 입장을 검증하고 HttpOnly 입장권을
설정한 뒤 303으로 각 탭의 원래 경로와 쿼리에 돌려보낸다. 서버가 입장을 거부해 대기 화면으로
돌려보내더라도 다시 전송하기까지 최소 3초를 둔다. 대기 중, 조회 실패, 요청 제한, 만료 상태에서는
자동 입장 요청을 보내지 않는다. 뒤로 가기로 복원된 화면은 상태를 다시 확인한다.
