...
현재 소스의 정상 종료는 진행 중인 HTTP 응답과 제한 시간 내의 큐 쓰기를 먼저 끝낸다.
`wrctl`은 이전 Compose 파일을 가진 설치에도 stop 유예 20초를 전달한다. 실제 I/O 제한
시간 초과나 강제 종료에는 기존 복구 대기가 적용된다. [종료 수정과 실제 키 ACK 검증](../evidence/beta-20260909-read-recovery.md).
