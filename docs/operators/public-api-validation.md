...
- 최대 1,000ms 시각 보정은 마지막 관측 시각을 유지해 quota/poll을 되감지 않는다. 새로운 요청 예산을 주거나 poll 마감을 앞당기지 않는다. 큐·입장권·epoch의 시계 검사는 별도로 유지한다.
- 함수 불일치, 기록 불일치, 1,000ms 초과 시계 역행, Valkey 오류는 `503 QUEUE_UNAVAILABLE`이며
  원본 우회로 전환하지 않는다.

제한 함수 `wr_public_guard_v3`는 대기열 runtime과 별도인 불변 라이브러리다. 기존 v1/v2
함수는 덮어쓰지 않으며, schema 1의 출처별 카운터·poll 일정·join 기록을 그대로 사용한다.
업그레이드 시 기존 데이터 역할을 정지하고 owner가 v3를 설치한 뒤 새 역할을 시작한다.
