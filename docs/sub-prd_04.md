...
### 2026-09-12 페이지 중심 연결 설정

- [x] 보호할 페이지에서 원본·보호 경로·상태 확인 주소 자동 입력, 기본 Waiting Room 서브도메인 제안과 다른 호스트 직접 입력. DNS 자동 적용은 제공하지 않는다.
- [x] 새 Room 입장권 TTL 60초, 기존 Room 보존, 예상시간 표시 선택 저장.
- [x] 관리자 Room/설치 브라우저 3엔진 9개 및 UI 단위 검사 PASS. [동작과 범위](operators/page-and-wait-progress.md).
