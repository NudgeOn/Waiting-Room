...
local function available(keys)
  return #keys == 11 and redis.call('HGET',keys[9],'dirty') == '0' and redis.call('HGET',keys[9],'mode') == 'ACTIVE'
...
local function prune(key,now)
  local ids=redis.call('ZRANGEBYSCORE',key,'-inf',now,'LIMIT',0,128)
  for _,id in ipairs(ids) do redis.call('ZREM',key,id) end
  return redis.call('ZCOUNT',key,'-inf',now)==0
end
...
local function cleanup(keys, now)
  ...
  local visitors=prune(keys[10],now)
  local idems=prune(keys[11],now)
  return visitors and idems and redis.call('ZCOUNT', keys[4], '-inf', now) == 0 and redis.call('ZCOUNT', keys[8], '-inf', now) == 0
end
redis.register_function('wr_i1_init',function(keys,args)
  ...
  if global and (global~=args[3] or redis.call('HGET',keys[9],'schema')~='1') then return redis.error_reply('WR_SCHEMA') end
  if room and (room~=args[1] or redis.call('HGET',keys[1],'room')~=args[4] or redis.call('HGET',keys[1],'schema')~='1') then return redis.error_reply('WR_SCHEMA') end
  ...
  if not global then redis.call('HSET',keys[9],'config',args[3],'schema','1','primary',args[2],'mode','ACTIVE','clock','0') end
  if not room then redis.call('HSET',keys[1],'config',args[1],'schema','1','room',args[4],'seq','0','mode','AUTO','primary',args[2],'clock','0') end
  commit(keys)
  return 'OK'
end)
redis.register_function{function_name='wr_i1_status', flags={'no-writes'}, callback=function(keys,args)
...
redis.register_function('wr_i1_command',function(keys,args)
  if not available(keys) then return redis.error_reply('WR_UNAVAILABLE') end
  local now=now_ms()
...
redis.register_function{function_name='wr_i1_capacity',flags={'no-writes'},callback=function(keys,args)
  if not available(keys) then return redis.error_reply('WR_UNAVAILABLE') end
  local now=now_ms()
  ...
  return cjson.encode({now=now,capacity={visitors=visitors,idempotency=idems,visitorCap=c.VisitorCap,idempotencyCap=c.IdempotencyCap,warning=visitors*5>=c.VisitorCap*4 or idems*5>=c.IdempotencyCap*4}})
end}
