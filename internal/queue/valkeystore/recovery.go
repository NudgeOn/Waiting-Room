...
func (s *Store) MaintainRecovery(ctx context.Context) (RecoveryState, error) {
	...
	raw, err := s.client.Do(ctx, s.client.B().Fcall().Function("wr_r4_recover").Numkeys(11).Key(s.keys...).Arg(primary, reason).Build()).ToString()
	if err != nil {
		return s.recovery, model.ErrUnavailable
	}
	...
	if state.Mode == "RECOVERY_HOLD" && state.Now >= state.UnsafeUntil && state.ValidationError == "" {
		raw, err = s.client.Do(ctx, s.client.B().Fcall().Function("wr_r4_validate").Numkeys(11).Key(s.keys...).Arg(state.Primary, strconv.FormatUint(state.Fence, 10)).Build()).ToString()
		if err != nil {
			return s.recovery, model.ErrUnavailable
		}
		state, err = decodeRecovery(raw)
...
func (s *Store) runtimeCall(ctx context.Context, read bool, args ...string) (Result, error) {
	...
	var err error
	if read {
		raw, err = s.client.Do(ctx, s.client.B().FcallRo().Function("wr_r4_"+operation).Numkeys(11).Key(s.keys...).Arg(values...).Build()).ToString()
	} else {
		raw, err = s.client.Do(ctx, s.client.B().Fcall().Function("wr_r4_command").Numkeys(11).Key(s.keys...).Arg(values...).Build()).ToString()
	}
	if err != nil {
