...
local function available(keys)
  return #keys == 11 and redis.call('HGET',keys[9],'schema') == '4' and redis.call('HGET',keys[9],'dirty') == '0' and redis.call('HGET',keys[9],'mode') == 'ACTIVE'
...
local function fenced(keys,args)
  if #keys~=11 or #args<2 then return false end
...
redis.register_function('wr_r4_init',function(keys,args)
  if #keys~=11 or #args~=4 or args[2]~=primary_id() then return redis.error_reply('WR_SCHEMA') end
  ...
  if global and (global~=args[3] or redis.call('HGET',keys[9],'schema')~='4') then return redis.error_reply('WR_SCHEMA') end
  if room and (redis.call('HGET',keys[1],'room')~=args[4] or redis.call('HGET',keys[1],'schema')~='4') then return redis.error_reply('WR_SCHEMA') end
  ...
  if not global then redis.call('HSET',keys[9],'config',args[3],'schema','4','primary',args[2],'mode','ACTIVE','clock','0','fence','1','unsafeUntil','0','leaseLifetime','60000') end
  ...
  redis.call('HSET',keys[1],'config',args[1],'schema','4','room',args[4],'seq','0','mode','HOLD','primary',args[2],'clock','0','configRevision','0','runtimeRevision','0','desired','','ready','0')
  redis.call('HSET',keys[9],'room:'..args[4],'1')
  if old_dirty~='1' then commit(keys) end
  return frame(keys,now_ms())
end)
redis.register_function('wr_r4_recover',function(keys,args)
  if #keys~=11 or #args~=2 or redis.call('HGET',keys[9],'schema')~='4' then return redis.error_reply('WR_SCHEMA') end
  local primary=primary_id()
  if primary~=args[1] then return redis.error_reply('WR_FENCED') end
  local now=now_ms()
...
redis.register_function{function_name='wr_r4_status', flags={'no-writes'}, callback=function(keys,args)
  ...
  if not c or redis.call('HGET',keys[1],'schema') ~= '4' then return redis.error_reply('WR_UNAVAILABLE') end
  local now=now_ms()
...
redis.register_function('wr_r4_command',function(keys,args)
  if not fenced(keys,args) then return redis.error_reply('WR_FENCED') end
  if not available(keys) then return redis.error_reply('WR_UNAVAILABLE') end
  local now=now_ms()
...
redis.register_function{function_name='wr_r4_capacity',flags={'no-writes'},callback=function(keys,args)
  if not fenced(keys,args) then return redis.error_reply('WR_FENCED') end
  if not available(keys) then return redis.error_reply('WR_UNAVAILABLE') end
  local now=now_ms()
...
redis.register_function('wr_r4_validate',function(keys,args)
  ...
  if not c or not room or redis.call('HGET',keys[1],'schema')~='4' then return validation_failed(keys,now,'room_schema') end
...
redis.register_function{function_name='wr_r4_metrics',flags={'no-writes'},callback=function(keys,args)
  if not fenced(keys,args) then return redis.error_reply('WR_FENCED') end
  if redis.call('HGET',keys[9],'schema')~='4' then return redis.error_reply('WR_UNAVAILABLE') end
  local now=now_ms()
