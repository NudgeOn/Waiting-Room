#!lua name=wr_queue_runtime_v6
-- SPDX-License-Identifier: Apache-2.0
-- ABI v6 keeps schema 5 and adds optional original join metadata to idempotency rows.
-- Existing rows/indexes are preserved. Published v5 code is never replaced.
...
local function answer(now, ticket, replay, join)
  return cjson.encode({now=now, ticket=ticket or cjson.null, replay=replay or '', join=join})
end
...
redis.register_function('wr_r6_init',function(keys,args)
...
redis.register_function('wr_r6_recover',function(keys,args)
...
redis.register_function{function_name='wr_r6_status', flags={'no-writes'}, callback=function(keys,args)
...
local function command(keys,args)
  ...
  if op == 'join' then
    ...
    if old then
      ...
      if old.join then
        return answer(now,raw and cjson.decode(raw) or old.join.ticket,old.replay,old.join)
      end
      if not raw then return redis.error_reply('WR_EXPIRED') end
    ...
    local join={ticket=t,idleTTL=c.IdleTTL}
    redis.call('HSET',keys[7],args[2],cjson.encode({id=t.ID,fingerprint=args[3],replay=args[5],join=join}))
    redis.call('ZADD',keys[8],now+c.IdempotencyTTL,args[2])
    return answer(now,t,args[5],join)
  end
  if op == 'promote' then
...
redis.register_function('wr_r6_command',function(keys,args)
  if not fenced(keys,args) then return redis.error_reply('WR_FENCED') end
  if not available(keys) then return redis.error_reply('WR_UNAVAILABLE') end
  local now=now_ms()
...
redis.register_function{function_name='wr_r6_capacity',flags={'no-writes'},callback=function(keys,args)
  if not fenced(keys,args) then return redis.error_reply('WR_FENCED') end
  if not available(keys) then return redis.error_reply('WR_UNAVAILABLE') end
  local now=now_ms()
...
redis.register_function('wr_r6_validate',function(keys,args)
  ...
  if phase=='idempotency' then
    ...
    for i=1,#ids,2 do
      ...
      if item.join then
        local j=item.join
        if type(j)~='table' then return validation_failed(keys,now,'join_replay') end
        local t=j.ticket
        if type(t)~='table' or not integer(j.idleTTL) or j.idleTTL<1 or t.ID~=item.id or t.State~='WAITING' or t.Epoch~=tonumber(redis.call('HGET',keys[1],'epoch')) or not integer(t.JoinedAt) or not integer(t.IdleUntil) or not integer(t.AbsoluteUntil) or t.IdleUntil~=t.JoinedAt+j.idleTTL or t.AbsoluteUntil<t.IdleUntil or score~=t.JoinedAt+c.IdempotencyTTL then return validation_failed(keys,now,'join_replay') end
      end
    end
    offset=offset+#ids/2
...
redis.register_function{function_name='wr_r6_metrics',flags={'no-writes'},callback=function(keys,args)
  if not fenced(keys,args) then return redis.error_reply('WR_FENCED') end
...
redis.register_function('wr_r6_configure_held',function(keys,args)
