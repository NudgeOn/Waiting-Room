...
test('Room draft persists across reload, conflicts are explicit, and mobile form fits',async({page,context,browserName})=>{
  ...
  try{
    ...
    await page.setViewportSize({width:360,height:900});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await page.getByLabel('분당 신규 입장 수',{exact:true}).fill('300');await page.getByRole('button',{name:'로고 제거',exact:true}).focus();await page.keyboard.press('Enter');await expect(page.getByAltText('선택한 로고 미리보기')).toHaveCount(0);await page.getByRole('button',{name:'초안 저장',exact:true}).click();await expect(page.getByRole('status').filter({hasText:'초안을 저장했습니다.'})).toContainText('초안을 저장했습니다.');
