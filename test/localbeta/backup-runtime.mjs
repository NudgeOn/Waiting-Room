...
try{
  ...
  const page=await context.newPage();await page.goto('https://127.0.0.1:29443/rooms/setup_room/operations');await page.getByRole('button',{name:'전체 대기열 초기화',exact:true}).click();await expect(page.getByRole('dialog')).toContainText('최소 60분 30초');await expect(page.getByRole('dialog')).toContainText('전체 대기열 1개');assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);const screens=path.join(os.tmpdir(),'wr-beta-recovery-qa');fs.mkdirSync(screens,{recursive:true});await page.screenshot({path:path.join(screens,'new-epoch-review-mobile.png'),fullPage:true});await page.keyboard.press('Escape');await expect(page.getByRole('dialog')).not.toBeVisible();await context.unrouteAll({behavior:'wait'});await browser.close();browser=null;
