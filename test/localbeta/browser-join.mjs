...
export async function browserJoinChecks({browser,origin,room,dataRequest,api}) {
 ...
 try {
  ...
  await page.route(origin+'/shop/lost-response',async route=>{
   if(!lost&&!route.request().isNavigationRequest()){
    const cookies=(await context.cookies()).map(c=>c.name+'='+c.value).join('; ');
    const out=await dataRequest('GET','/shop/lost-response',undefined,{Accept:'text/html',Cookie:cookies});
  ...
  await page.goto(origin+'/shop/lost-response');
  ...
  await page.screenshot({path:'/private/tmp/wr-beta4-https-join-retry.png',fullPage:true});
 ...
 console.log('PASS: actual HTTPS first browser response discarded with all cookies, same-ticket keyboard recovery, Secure/HttpOnly intent, 5 simultaneous fresh tabs with independent targets, prepare contracts and mobile axe');
}
