...
export async function eventChecks({page,room,mark,screens}){
 ...
 const tab=name=>page.getByRole('navigation',{name:'대기열 화면'}).getByRole('link',{name,exact:true}).click();
 await tab('입장 예약');await expect(page).toHaveURL(new RegExp(`/rooms/${room.id}/schedule$`));
 ...
 async function fill(offsets){
  ...
  for(const [i,label] of ['사전 대기 시작','입장 시작','새 대기 접수 종료'].entries()){
 ...
 await tab('입장 운영');await page.getByRole('button',{name:'입장 잠시 멈춤',exact:true}).click();await expect(page.getByRole('status')).toContainText('명령을 저장했습니다.');await tab('입장 예약');await expect(row).toContainText('수동 변경으로 일시정지');
 ...
 await expect(page.getByRole('button',{name:new RegExp(room.name)})).toContainText('남은 사람 입장 후 종료 중');
 ...
 await tab('입장 운영');await page.getByRole('button',{name:'입장 잠시 멈춤',exact:true}).click();await expect.poll(async()=>(await read()).view.runtimes[0].runtime.mode).toBe('HOLD');
