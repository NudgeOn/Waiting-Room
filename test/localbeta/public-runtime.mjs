...
try{
  ...
  const unavailableContext=await browser.newContext({ignoreHTTPSErrors:true,locale:'ko-KR',viewport:{width:360,height:900}});const unavailablePage=await unavailableContext.newPage();const stopped=await unavailablePage.goto(origin+'/shop');assert.equal(stopped.status(),503);await expect(unavailablePage.getByRole('heading',{name:'잠시 입장을 멈췄어요'})).toBeVisible();await unavailablePage.keyboard.press('Tab');await expect(unavailablePage.getByRole('link',{name:'다시 확인하기'})).toBeFocused();const audit=await new AxeBuilder({page:unavailablePage}).withTags(['wcag2a','wcag2aa','wcag21aa']).analyze();assert.deepEqual(audit.violations.map(v=>({id:v.id,targets:v.nodes.map(n=>n.target)})),[]);assert.equal(await unavailablePage.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await unavailablePage.screenshot({path:'/private/tmp/wr-beta2-draining-mobile.png',fullPage:true});await unavailableContext.close();await page.goto(origin+'/shop/cart?resume=drain');await expect(page.locator('main')).toBeVisible();await command('auto');await until(async()=>{const d=(await request(29473,'/config/delivery')).body;return d.nodes.find(n=>n.id==='coordinator')?.rooms[0]?.ready>=1;});
