...
try{
  ...
  if(process.env.WR_TEST_THEME_LOGO==='1')room.theme.logoImage=Buffer.concat([fs.readFileSync('test/fixtures/theme-logo.png'),Buffer.from('private-upload-metadata')]).toString('base64');
  ...
  if(process.env.WR_TEST_THEME_LOGO==='1'){
    ...
    assert.equal(logo.bytes.toString('base64'),saved.body.rooms[0].theme.logoImage);assert.equal(logo.bytes.includes(Buffer.from('private-upload-metadata')),false);
    const head=await dataRequest('HEAD',logoURL);assert.equal(head.status,200);assert.equal(head.bytes.length,0);
    const wrong=await dataRequest('POST',logoURL);assert.equal(wrong.status,405);assert.equal(wrong.headers.allow,'GET, HEAD');
    await expect(page.locator('img.room-logo')).toBeVisible();await expect.poll(()=>page.locator('img.room-logo').evaluate(el=>el.complete&&el.naturalWidth===32)).toBe(true);
    const logoAxe=await new AxeBuilder({page}).withTags(['wcag2a','wcag2aa','wcag21a','wcag21aa','wcag22aa']).analyze();assert.deepEqual(logoAxe.violations.map(v=>({id:v.id,targets:v.nodes.map(n=>n.target)})),[]);
    await page.setViewportSize({width:320,height:800});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await page.screenshot({path:path.join(os.tmpdir(),'wr-beta8-logo-public-mobile.png'),fullPage:true});await page.setViewportSize({width:360,height:900});
    console.log('PASS: decoded/re-encoded logo saved, signed, rendered as same-origin PNG at 320px with strict CSP, image GET/HEAD/method contract and axe 0 violations');
  }
  ...
  if(process.env.WR_TEST_KEYS==='1'){
    ...
    for(const [operation,phase,generation] of [['stage','staged',1],['activate','active',2]]){
      ...
      await rotationPage.goto(oldReturnURL);await expect(rotationPage.locator('main')).toBeVisible();if(process.env.WR_TEST_THEME_LOGO==='1')await expect.poll(()=>rotationPage.locator('img.room-logo').evaluate(el=>el.complete&&el.naturalWidth===32)).toBe(true);
    }
