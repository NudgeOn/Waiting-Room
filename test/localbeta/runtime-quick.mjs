...
try{
 ...
 const page=await context.newPage(),browserErrors=[];page.on('pageerror',()=>browserErrors.push('pageerror'));page.on('console',m=>{if(['error','warning'].includes(m.type())&&!/Failed to load resource: the server responded with a status of (401|403|404|409|412)/.test(m.text()))browserErrors.push('unexpected console '+m.type());});await page.goto(admin);await expect(page.getByRole('heading',{name:'관리자 세션'})).toBeVisible();await expect(page).toHaveTitle('NudgeOn Waiting Room · 관리자 콘솔');
 await runtimeChecks({page,browser,mark,screens,docker});
 if(process.env.WR_TEST_LOCAL_RECOVERY==='1'){
  await page.getByRole('button',{name:'대기열 관리',exact:true}).click();await expect(page.getByRole('heading',{name:'최근 변경 기록',exact:true})).toBeVisible();await expect(page.getByText('대기열 복구 · 안전 대기 시작',{exact:true})).toBeVisible();await expect(page.getByText('대기열 복구 · 검증 후 복구',{exact:true})).toBeVisible();assert.equal(await page.locator('.audit-list').innerText().then(t=>t.includes('Invalid Date')),false);
  ...
  await page.screenshot({path:path.join(screens,'runtime-recovery-audit-mobile.png'),fullPage:true});await page.getByRole('button',{name:'대시보드로',exact:true}).click();mark('recovery audit UI shows held/resumed outcomes and server occurrence timestamps');
 }
